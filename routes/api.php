...
Route::get('/check-ip', function (Request $request) {
    ...
    Route::middleware(['validate.api.client'])->group(function () {
        ...
        Route::prefix('search')->group(function () {
            ...
            Route::get('/get-filterdata-by-search-result', [SearchEngineController::class, 'getFilterDataBySearchResult']);

            // POST apply filters and get property list
            Route::post('/apply-filters', [SearchEngineController::class, 'applyFilters']);
        });


        ############# new ########
            // 1. Global Search API
                Route::post('/global-search', [SearchEngineController::class, 'globalSearch']);

                // 2. Global Filters API
                Route::post('/global-filters', [SearchEngineController::class, 'globalFilters']);

                // 3. Apply Filter API
                Route::post('/apply-filter', [SearchEngineController::class, 'applyFilter']);
        ############ end new #########
        ...
        # 14 Oct 2025
        Route::middleware(['allow.admin_company'])->get('/get-all-project-listings-by-companyoradmin-token', [ProjectlistingController::class, 'getAllProjectsListingByCompanyOrAdminToken']);
        ### No Auth ###
