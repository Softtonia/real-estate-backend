...
class MembershipWebhookService
{
    ...
    private function processPayload(MembershipWebhookEvent $event): void
    {
        ...
        if (!$order) {
            throw new \RuntimeException('Local membership order not found for Razorpay order id: ' . $razorpayOrderId);
        }
    ...
    private function storePaymentFromWebhook(
        ...
        return DB::transaction(function () use ($order, $paymentEntity, $eventName, $razorpayPaymentId) {
