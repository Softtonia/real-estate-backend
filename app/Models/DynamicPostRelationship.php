...
class DynamicPostController extends Controller
{
    ...
    private function normalizeRelationshipPostTypeInputs(array $validated): array
    {
        ...
        $extractIds = function (mixed $value) use (&$extractIds): array {
            ...
            // Single dropdown option object: { id: 18, value: 18, label: "Developer" }
        ...
        foreach ($input as $key => $item) {
            ...
            if (is_array($item)) {
                // Supported shapes:
                // relationship_post_types: [{ post_type_id: 3, post_ids: [18, 19] }]
                // relationship_post_types: { "3": { post_ids: [18, 19] } }
                // relationship_post_types: { "3": [18, 19] }
                // relationship_post_types: [{ value: 18, label: "Developer" }]
                if (array_key_exists('post_ids', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['post_ids']));
                    continue;
                }

                if (array_key_exists('related_post_ids', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['related_post_ids']));
                    continue;
                }

                if (array_key_exists('ids', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['ids']));
                    continue;
                }

                if (array_key_exists('post_id', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['post_id']));
                    continue;
                }

                if (array_key_exists('related_post_id', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['related_post_id']));
                    continue;
                }

                if (array_key_exists('selected_post_ids', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['selected_post_ids']));
                    continue;
                }

                if (array_key_exists('selected_posts', $item)) {
                    $rawPostIds = array_merge($rawPostIds, $extractIds($item['selected_posts']));
                    continue;
                }
            }
