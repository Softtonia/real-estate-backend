...
class TaxonomyController extends Controller
{
    ...
    public function update(
         ...
         */
            ...
            $childTaxonomyIds = array_values(
                array_diff($childTaxonomyIds, [$taxonomyId])
            );
         ...
         */
            ...
            if (!empty($relationshipIds)) {
                ...
                if (!empty($invalidRelationshipIds)) {
                    ...
                    return response()->json([
                        'status' => false,
                        'message' => 'Invalid taxonomy selection.',
                        'errors' => [
                            'relationships' => [
         ...
         */
            ...
            foreach ($childTaxonomyIds as $childId) {
                $reverseExists = DB::table(
                    'taxonomy_relationships'
                )
                    ->where(
            ...
            if (array_key_exists('hierarchical', $validated)) {
                $updateData['hierarchical'] =
                    $request->boolean('hierarchical');
            }
    ...
    private function validateRelationshipData(
        ...
        if ($isParentMode && !empty($parentIds)) {
            return response()->json([
                'status' => false,
                'message' => 'Invalid relationship payload.',
                'errors' => [
                    'parent_ids' => [
                        'Send child_taxonomy_ids when assigning child taxonomies.',
                    ],
                ],
            ], 422);
        }

        if (!$isParentMode && !empty($childTaxonomyIds)) {
            return response()->json([
                'status' => false,
                'message' => 'Invalid relationship payload.',
                'errors' => [
                    'child_taxonomy_ids' => [
                        'Send parent_ids when assigning parent taxonomies.',
                    ],
                ],
            ], 422);
        }
        ...
        if (
            ...
            return response()->json([
                'status' => false,
                'message' => 'At least one parent taxonomy is required.',
                'errors' => [
                    'parent_ids' => [
        ...
        if ($currentTaxonomy) {
            $currentId = (int) $currentTaxonomy->id;

            if (in_array($currentId, $parentIds, true)) {
                return response()->json([
                    'status' => false,
