...
class PostTypeController extends Controller
{
    ...
    {
        $validated = $request->validate(['ids'=>['required','array','min:1'],'ids.*'=>['integer','exists:post_types,id']]);
        $forceDeleted = PostType::onlyTrashed()->whereIn('id',$validated['ids'])->get()->each(function($pt){if(!$pt->is_default) $pt->forceDelete();});
    ...
    private function formatPostType($pt): array
    {
        ...
        return [
            ...
            'menu_order'=>$pt->menu_order,
            'taxonomies'=>$pt->relationLoaded('taxonomies')?$pt->taxonomies->map(fn($t)=>[
                'id'=>$t->id,
                'name'=>$t->name,
                'slug'=>$t->slug,
            ...
            'created_by_user'=>$creator?['id'=>$creator->id,'name'=>$this->getUserFullName($creator),'email'=>$creator->email ?? null,'role'=>$this->getUserRoleName($creator)]:null,
    ...
    private function getUserRoleName($user): ?string
    {
        if(method_exists($user,'roles')){
            try{$roleName=$user->roles()->pluck('name')->first(); if(!empty($roleName)) return $roleName;}catch(\Exception $e){}
        }
        ...
        if(isset($user->role_id)) try{$role=DB::table('roles')->where('id',$user->role_id)->first(); return $role->name ?? $role->role_name ?? null;}catch(\Exception $e){return null;}
        return null;
    }
}