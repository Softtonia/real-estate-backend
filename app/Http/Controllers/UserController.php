...
// }
    ...
    // this is for top features
    // public function createTopFeatures(Request $request)
    // {
    //     try {
    ...
    //             TopFeature::create($insertData);
    //         }

    //         return response()->json(['message' => 'Top feature set successfully'], 200);
    //     } catch (\Exception $e) {
    //         return response()->json(['error' => 'Failed.' . $e->getMessage()], 500);
    //     }
    // }
    public function createTopFeatures(Request $request)
    {
        try {
            ...
            // Ensure $section_name is always an array
            if (!is_array($section_name)) {
                $section_name = [$section_name]; // Convert string to array
            }
    ...
    // this is for top features
    public function listingOfTopFeatures(Request $request)
    {
        try {
            ...
            $for = null;
            if (isset($request->for) && $request->for == 'agent') {
                $for = $request->for;

                $agentIds = TopFeature::where('section_name', $section_name)->where('agent_id', '!=', null)->pluck('agent_id')->toArray();
            ...
            $for = null;
            if (isset($request->for) && $request->for == 'project') {
                $for = $request->for;

                $projectIds = TopFeature::where('section_name', $section_name)->where('project_id', '!=', null)->pluck('project_id')->toArray();
            ...
            $for = null;
            if (isset($request->for) && $request->for == 'property') {
                $for = $request->for;

                $propertyIds = TopFeature::where('section_name', $section_name)->where('property_id', '!=', null)->pluck('property_id')->toArray();
                ...
                $propertiesData = $properties->map(function ($property) use ($baseURL, $basePath) {
                    ...
                    $propertyData = [
                        ...
                        // ✅ Added Country, State, and City Names & Table Names
                        'country_id' => $property->country_id,
                        'country_name' => optional($property->country)->name,
                        'country_table_name' => 'countries', // ✅ Show table name

                        'state_id' => $property->state_id,
                        'state_name' => optional($property->state)->name,
                        'state_table_name' => 'states', // ✅ Show table name

                        'city_id' => $property->city_id,
                        'city_name' => optional($property->city)->name,
                        'city_table_name' => 'cities', // ✅ Show table name
                        ...
                        // ✅ Added Created By & Updated By Role Names
                        'created_by_role_name' => optional($property->createdBy)->role->name ?? 'N/A',
                        'updated_by_role_name' => optional($property->updatedBy)->role->name ?? 'N/A',

                        'listed_by' => optional(optional($property->user)->role)->name,
                        'featured_image' => $property->featured_image ?
                            $this->correctFilePath($property->featured_image, $baseURL, $basePath, 'featured_image') : null,
                        ...
                        // ✅ Fixed Incorrect Time Formatting
                        'date' => date('d m Y', strtotime($property->created_at)),
                        'time' => date('h:i A', strtotime($property->created_at)), // Corrected format
                        'timestamp' => date('d m Y h:i A', strtotime($property->created_at)), // Corrected format
    ...
    // this is for top features
    public function topFeaturesListing(Request $request)
    {
        try {
            ...
            $for = null;
            if (isset($request->for) && $request->for == 'agent') {
                $result = TopFeature::where('agent_id', $request->agent_id)->get();
            }


            $for = null;
            if (isset($request->for) && $request->for == 'project') {
                $result = TopFeature::where('project_id', $request->project_id)->get();
            }
            ...
            $for = null;
            if (isset($request->for) && $request->for == 'property') {
                $result = TopFeature::where('property_id', $request->property_id)->get();
            }
