...
public function update(Request $request)
{
    ...
    try {
        ...
        $page = Page::where('page', 'PropertyValuation')->first();

        if (!$page) {
            // If page doesn't exist, create a new one
            $page = new Page();
            $page->page = 'PropertyValuation';
        }
...
public function index(Request $request)
{
    try {
        ...
        $pages = Page::where('page', 'PropertyValuation')->get();

        // Check if there are any pages
        if ($pages->isEmpty()) {
            return response()->json(['error' => 'No pages found with the name "PropertyValuation"'], 404);
        }
        ...
        // Log the specific error message for debugging purposes
        \Log::error('Error listing PropertyValuation pages: ' . $th->getMessage());
        return response()->json(['error' => 'An error occurred while listing PropertyValuation pages'], 500);
    }
}
