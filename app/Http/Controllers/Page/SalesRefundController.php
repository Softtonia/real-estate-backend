...
public function update(Request $request)
{
    ...
    try {
        ...
        $page = Page::where('page', 'Sales&Refund')->first();

        if (!$page) {
            // If page doesn't exist, create a new one
            $page = new Page();
            $page->page = 'Sales&Refund';
        }
...
public function index(Request $request)
{
    try {
        ...
        $pages = Page::where('page', 'Sales&Refund')->get();

        // Check if there are any pages
        if ($pages->isEmpty()) {
            return response()->json(['error' => 'No pages found with the name "Sales&Refund"'], 404);
        }
        ...
        // Log the specific error message for debugging purposes
        \Log::error('Error listing Sales&Refund pages: ' . $th->getMessage());
        return response()->json(['error' => 'An error occurred while listing Sales&Refund pages'], 500);
    }
}
