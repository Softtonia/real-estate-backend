...
{
    try {
        // Fetch pages with the name "Privacy Policy"
        $pages = Page::where('page', 'Privacy Policy')->get();

        // Check if there are any pages
        if ($pages->isEmpty()) {
            return response()->json(['error' => 'No pages found with the name "Privacy Policy"'], 404);
        }
        ...
        // Log the specific error message for debugging purposes
        \Log::error('Error listing Privacy Policy pages: ' . $th->getMessage());
        return response()->json(['error' => 'An error occurred while listing Privacy Policy pages'], 500);
    }
}
