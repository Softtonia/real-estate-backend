...
class PropertylistingController extends Controller
{
    ...
    //                     if (isset($request->{$modelField['model']}) &&
    ...
    public function store(Request $request)
{
    try {
        ...
        // Set `live_status` default to "Under Review"
        $liveStatus = $request->live_status ?? 'Under Review';

        // Non-admin users should always have "Under Review" as live_status
        if ($user->role->name !== 'admin') {
        ...
        // Validate status_reason only when live_status is "Reject"
        $request->validate([
            'status_reason' => $liveStatus === 'Reject' ? 'required|string' : 'nullable',
        ]);
        ...
        $propertyData = [
            ...
            'state_id' => $request->state_id,
            'city_id' => $request->city_id,
            'status_reason' => $liveStatus === 'Reject' ? $request->status_reason : null,
            'created_by' => $user->id,
        ...
        foreach (['property_video', 'virtual_tour', 'video_thumbnail', 'brochure'] as $fileField) {
            if ($request->hasFile($fileField)) {
                ...
                $fileName = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/' . $fileField), $fileName);
                $propertyData[$fileField] = '/uploads/' . $fileField . '/' . $fileName; // ✅ Store path
            }
        }
    ...
    public function indexByAdmin(Request $request)
{
    try {
        ...
        $projects = PropertyList::with([
                'location', 'user', 'propertyType', 'purpose',
                'property', 'propertystatus', 'customFieldValues.customField',
                'customFieldValues.customFieldOption', 'importKeywords',
                'developer.userDetails', 'createdBy.role', 'updatedBy.role'
            ])
    ...
    public function index(Request $request)
{
    try {
        ...
        $properties = PropertyList::with([
                'location', 'user', 'propertyType', 'purpose', 'property', 'propertystatus',
                'project', 'customFieldValues.customField', 'customFieldValues.customFieldOption', 'importKeywords'
            ])
    ...
    public function getdatabyId(Request $request)
    {
        try {
            ...
            $property = PropertyList::with([
                    'location', 'user', 'propertyType', 'purpose', 'property',
