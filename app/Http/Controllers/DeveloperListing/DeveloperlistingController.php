...
class DeveloperlistingController extends Controller
{
    ...
    public function getAssociatedProjectsWithDeveloper(Request $request)
    {
        try {
            $baseURL = config('app.url');
            $basePath = public_path();
            ...
            $developerQuery = DeveloperList::with([
            ...
            if ($user->role->name !== 'admin') {
                ...
                $developerQuery->where('user_id', $user->id);
            }
            
            
            $developer =$developerQuery ->find($request->developer_id);

            if (!$developer) {
                return response()->json([
                    'status' => false,
                    'message' => 'Developer not found or access denied.',
                    'developer_info' => null,
                    'data' => [],
                ], 200);
            }
            ...
            $developerInfo = $developer ? [
                ...
                'propertyStatuses' => $developerPropertyStatuses ?? null,
                'propertyTypes' => $developerPropertyTypes ?? null,
                'country_id' => $developer->country_id,
                'country_name' => optional($developer->country)->name,
                'state_id' => $developer->state_id,
                'state_name' => optional($developer->state)->name,
