...
class ExcludeOwnerMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            $user = \App\Models\User::where('api_token', $requestToken)->with('role')->firstOrFail();
            Auth::setUser($user);
        } catch (\Exception $e) {
        ...
        if (Auth::check() && Auth::user()->role && Auth::user()->role->name === 'owner') {
            return response()->json(['error' => 'Access denied for owner role.'], 403);
        }
