...
class LogAndBlockIpMiddleware
{
    ...
    //     // 🔄 Check if user has logged in from another IP previously
    //     if ($userId) {
    //         $existingOtherIpLog = UserIpLog::where('user_id', $userId)
    //             ->where('ip_address', '!=', $ip)
    //             ->first();

    //         if ($existingOtherIpLog) {
    //             // 👇 Option 1: Update the old IP to current IP (overwrite)
    //             $existingOtherIpLog->ip_address = $ip;
    //             $existingOtherIpLog->save();

    //             // ✅ Optionally skip logging again below
    //         }
    //     }

    //     // 📝 Update user_id if available but missing
    //     if ($ipLog && !$ipLog->user_id && $userId) {
    //         $ipLog->user_id = $userId;
    //         $ipLog->save();
    //     }

    //     // 🌍 Log IP if not already recorded
    //     if (!$ipLog) {
    //         $location = Location::get($ip);
    //         UserIpLog::create([
    //             'user_id' => $userId,
